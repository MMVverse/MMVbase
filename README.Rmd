...
This R package contains general functions, originally written for malaria pharmacometric 
modeling and simulation activities, which can be useful in a broader set of use cases. 
The package is part of the MMVverse, a collection of open source R packages developed by MMV.

## Installation

You can install the development version of MMVbase from [GitHub](https://github.com/) with:
...
```
## User guide
To learn more about the package and its functionalities, you can access the package 
vignette by running the following code after installing the package:
...
The package MMVbase contains common use functions used in modeling and simulation 
projects. The package collects 
...
| Function                  | Description                                                                 |
|---------------------------|-----------------------------------------------------------------------------|
| `IsInClosedInterval`      | Check if value is in the closed interval \code{[low, high]}                  |
| `IsInOpenInterval`        | Check if value is in the open interval \code{(low, high)}                   |
| `IsInLeftOpenInterval`    | Check if value is in the left open interval \code{(low, high]}              |
| `IsInRightOpenInterval`   | Check if value is in the right open interval \code{[low, high)}             |
...
We recommend using interval functions in R scripts and other functions implementations instead of 
...
goes in more than one step and can have several logical branches:
  ...
  - [TODO] report tables can be exported to different table formats, such as Excel sheets, or xtable. 
...
of the other. Here's a summary of the function:
...
The translational family of functions provides various formulas for converting 
concentrations between different matrices, estimating hepatic clearance, and 
calculating hepatic extraction ratios. These functions are essential for 
